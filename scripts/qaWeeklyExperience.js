/**
 * QA: Phase-2D Weekly Experience Engine
 *
 * Checks:
 * - ok=true
 * - narrative is 8–12 lines
...
async function main() {
  ...
  const w = await fetch(`${base}/windows`, {
    ...
    body: JSON.stringify({ user_id: 1, chart_id: 1, scope: 'weekly', timezone: 'Asia/Kolkata', date, chart_data }),
  });
  const wj = await w.json();
  const windowId = Number(wj?.window?.id);
  assert(wj.ok && windowId, 'weekly window creation failed');
  ...
  const out = await (await fetch(`${base}/weekly-experience/${windowId}`)).json();
  assert(out.ok === true, 'weekly-experience ok=false');

  const narrative = String(out?.narrative || '');
  const lines = narrative.split('\n').map(s => s.trim()).filter(Boolean);
  console.log('lines', lines.length);
  assert(lines.length >= 8 && lines.length <= 12, `narrative must be 8–12 lines (got ${lines.length})`);

  // Continuity anchor must exist (one of the anchor phrases)
